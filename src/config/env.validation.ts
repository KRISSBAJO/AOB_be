...
export function validateEnv(config: EnvInput) {
  ...
  return {
    ...
    BACKGROUND_JOBS_DISABLED: config.BACKGROUND_JOBS_DISABLED,
    EMAIL_DELIVERY_POLL_MS: Number(config.EMAIL_DELIVERY_POLL_MS ?? 30_000),
    PUBLIC_BOOKING_WORKSPACE_ID: config.PUBLIC_BOOKING_WORKSPACE_ID,
    PUBLIC_BOOKING_INTERNAL_EMAIL:
      config.PUBLIC_BOOKING_INTERNAL_EMAIL ??
      config.EMAIL_FROM ??
      config.MAIL_FROM,
    ...
    AWS_SECRET_ACCESS_KEY:
      config.AWS_SECRET_ACCESS_KEY ?? config.S3_SECRET_ACCESS_KEY,
